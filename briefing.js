 ...
 * Strictly following provided template and logic rules.
 */

function generateBriefing() {
    const selectedDate = document.getElementById('dateSelect').value;
    const dayStaff = scheduleData[selectedDate];
    if (!dayStaff) { alert("Data not found"); return; }
    ...
    // Sort by exit time for HACCP assignment
    ...
    if (haccpPool[1]) b += `${getExit(haccpPool[1])} *Limpeza de Espelhos e vidros:* ${haccpPool[1].alias}\n`;
    
    const last = haccpPool[haccpPool.length-1] || { alias: "---" };
    b += `${getExit(last)} *Fecho da sala:* ${last.alias}\n\n`;
