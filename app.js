/* ZENITH MANAGER - VERSION 1.5
   Logic: Navigation, CRUD with Multi-click Prevention for Staff Data
*/
...
async function confirmDelete() {
    ...
    const originalKey = document.getElementById('editOriginalKey').value;
    if (!confirm(`Are you sure you want to remove ${originalKey}?`)) return;
    ...
    try {
        await fetch(SCRIPT_URL, { method: 'POST', mode: 'no-cors', body: JSON.stringify({ action: 'delete', originalKey }) });
        setTimeout(() => {
            alert("Staff Removed");
            closeStaffModal();
            loadData();
